  ...
  return (
    <div className={`fixed inset-0 z-[320] ${open ? "" : "pointer-events-none"}`} aria-hidden={!open}>
      ...
      <aside
        ...
        className={`absolute right-0 top-0 flex h-full w-[min(90vw,380px)] flex-col bg-[#e9f4e5] text-[#422900] shadow-[0_28px_90px_rgba(66, 41, 0,0.28)] transition-transform duration-300 ease-out md:w-[max(380px,45vw)] ${
      ...
      >
        <header className="flex items-center justify-between border-b border-[#422900]/12 px-6 py-5">
          ...
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-full border border-[#422900]/12 bg-[#e9f4e5] text-[#422900] transition-colors hover:bg-[#c6dac0]"
          >
            ✕
          </button>
        </header>
        ...
        <footer className="flex items-center gap-3 border-t border-[#422900]/12 px-6 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-12 flex-1 rounded-full bg-[#422900] text-sm font-semibold text-[#e9f4e5] transition-colors hover:bg-[#301f00]"
          >
            Show {resultCount}
          </button>
        </footer>
...
export function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-7 last:mb-0">
      <p className="mb-3 text-[12px] font-bold uppercase tracking-wide text-[#628454]">{title}</p>
      {children}
    </div>
  );
}
    ...
    <button
      ...
      className={[
        ...
        active
          ...
          : "border-[#422900]/16 bg-[#e9f4e5] text-[#422900] hover:border-[#422900]/40",
      ].join(" ")}
    >
      {children}
    </button>
...
export function FilterPill({
  ...
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-[#422900] py-1 pl-3 pr-1.5 text-[13px] font-semibold text-[#e9f4e5]">
      {children}
      <button
        type="button"
        onClick={onRemove}
        aria-label="Remove filter"
        className="grid h-5 w-5 place-items-center rounded-full bg-[#e9f4e5]/20 text-[11px] transition-colors hover:bg-[#e9f4e5]/35"
      >
        ✕
      </button>
    </span>
