    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.4fr]">
      ...
      <div className="spabox-card flex flex-col p-5 sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[#422900]/14 bg-[#e9f4e5] px-4 text-sm font-semibold text-[#422900] transition-colors hover:border-[#422900]/40"
          ...
          >
            ...
            Filters
            {activeFilterCount > 0 && (
              <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#422900] px-1.5 text-[11px] font-bold text-[#e9f4e5]">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        <label className="relative mb-3 block">
          ...
          <SearchIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#628454]" />
          <input
            type="search"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder={searchPlaceholder}
            className="h-11 w-full rounded-full border border-[#422900]/14 bg-[#e9f4e5] pl-12 pr-4 text-sm font-medium text-[#422900] outline-none transition-colors placeholder:text-[#628454] focus:border-[#422900]/40"
          />
        </label>
        ...
        <p className="mb-2 text-xs font-medium text-[#4e3900]/68">
        ...
        <div className="no-scrollbar max-h-[420px] space-y-2 overflow-y-auto lg:max-h-[560px]">
          {filtered.length === 0 ? (
            <div className="rounded-[18px] border border-dashed border-[#422900]/20 p-6 text-center">
              <p className="m-0 text-sm font-medium text-[#4e3900]/72">No treatments match.</p>
              <button
                type="button"
                onClick={() => {
                  clearAll();
                ...
                className="mt-2 text-sm font-semibold text-[#628454] underline-offset-4 hover:underline"
              >
                Reset
              </button>
            </div>
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    active
                      ? "border-[#422900] bg-[#422900] text-[#e9f4e5]"
                      : "border-[#422900]/12 bg-[#e9f4e5]/68 text-[#422900] hover:border-[#422900]/35",
                  ].join(" ")}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="m-0 truncate text-sm font-semibold">{treatment.name}</p>
                      <p className={`m-0 mt-0.5 text-xs ${active ? "text-[#e9f4e5]/72" : "text-[#4e3900]/68"}`}>
                        {treatment.duration_minutes ? `${treatment.duration_minutes} min · ` : ""}
                        {brand.currency} {treatment.default_price}
                      </p>
                    </div>
                    {active && (
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#e9f4e5] text-xs text-[#422900]">
                        ✓
                      </span>
                    )}
                    {treatment.image_url && (
      ...
      <div className="spabox-card p-4 sm:p-6">
        ...
        <button
          ...
          className="mt-6 inline-flex w-full items-center justify-center gap-2 bg-[#422900] px-5 py-3 text-sm font-semibold text-[#e9f4e5] transition-colors hover:bg-[#301f00] disabled:cursor-not-allowed disabled:opacity-50"
        >
          {submitting
            ? "Confirming…"
            : selectedSlot
